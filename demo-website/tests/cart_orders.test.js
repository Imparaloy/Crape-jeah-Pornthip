...
beforeAll(async () => {
  ...
  await request(app).post('/api/register').send({ name: 'buyer', password: 'pass123' }).expect(201);
  customerToken = (await request(app).post('/api/login').send({ name: 'buyer', password: 'pass123' }).expect(200)).body.token;
  await request(app).post('/api/register').send({ name: 'seller2', password: 'pass123', role: 'seller' }).expect(201);
  const sellerToken = (await request(app).post('/api/login').send({ name: 'seller2', password: 'pass123' }).expect(200)).body.token;
