...
beforeAll(async () => {
  ...
  await request(app).post('/api/register').send({ name: 'customer', password: 'pass123' }).expect(201);
  await request(app).post('/api/register').send({ name: 'seller', password: 'pass123', role: 'seller' }).expect(201);
  sellerToken = (await request(app).post('/api/login').send({ name: 'seller', password: 'pass123' }).expect(200)).body.token;
});
...
test('GET /api/menus/:id -> 200 returns created menu', async () => {
  const response = await request(app).get(`/api/menus/${createdMenuId}`).expect(200);
